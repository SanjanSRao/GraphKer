...
class CAPECInserter:
    ...
    def query_capec_reference_script(self, file):
        capecs_cypher_file = open(r"CypherScripts\\CAPECs_reference.cypher", "r")
        ...
        query = query.replace('capecReferenceFilesToImport', file_path)
        try:
    ...
    def query_capec_attack_script(self, file):
        capecs_cypher_file = open(r"CypherScripts\\CAPECs_attack.cypher", "r")
        query = capecs_cypher_file.read()
        file_paths = os.path.normpath(os.path.join(self.import_path, file))
        relative_path = 'C:/'
        file_path = os.path.relpath(file_paths, relative_path)
        file_path = file_path.replace(os.sep, '//')
        file_path = '"%s"' % file_path
        query = query.replace('capecAttackFilesToImport', file_path)
        try:
            with self.driver.session() as session:
                session.run(query)
        except exceptions.CypherSyntaxError as e:
    ...
    def query_capec_category_script(self, file):
        capecs_cypher_file = open(r"CypherScripts\\CAPECs_category.cypher", "r")
        query = capecs_cypher_file.read()
        file_paths = os.path.normpath(os.path.join(self.import_path, file))
        relative_path = 'C:/'
        file_path = os.path.relpath(file_paths, relative_path)
        file_path = file_path.replace(os.sep, '//')
        file_path = '"%s"' % file_path
        query = query.replace('capecCategoryFilesToImport', file_path)
    ...
    def query_capec_view_script(self, file):
        capecs_cypher_file = open(r"CypherScripts\\CAPECs_view.cypher", "r")
        query = capecs_cypher_file.read()
        file_paths = os.path.normpath(os.path.join(self.import_path, file))
        relative_path = 'C:/'
        file_path = os.path.relpath(file_paths, relative_path)
        file_path = file_path.replace(os.sep, '//')
        file_path = '"%s"' % file_path
        query = query.replace('capecViewFilesToImport', file_path)
